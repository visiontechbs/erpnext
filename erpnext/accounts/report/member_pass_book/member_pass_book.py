...
def get_gl_entries(filters, accounting_dimensions):
	...
	# +AU
	GL_Entry = ""
	if filters.get("gl_entry"):
		GL_Entry = filters.gl_entry
	#END OF +AU
	...
	gl_entries = frappe.db.sql(
		...
		from `tabGL Entry`
		where name > '{GL_Entry}'
		AND company=%(company)s {conditions}
		{distributed_cost_center_query}
		{order_by_statement}
		""".format(
			...
			order_by_statement=order_by_statement, GL_Entry = format(GL_Entry)
		),
		filters, as_dict=1)
