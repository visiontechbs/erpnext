...
def	 fetch_subscription_list():
  try:
      #  subscription_plan_list = frappe.db.sql("Select name from `tabSubscription Plan`")
       subscription_plan_list = frappe.db.sql("""Select name from `tabMembership` t1 INNER JOIN `tabContribution List Table` t2 ON t2.parent = t1.name""")
       loan_type_list = frappe.db.sql("Select name from `tabLoan Type`")
       return subscription_plan_list, loan_type_list
  except:
	   return None
